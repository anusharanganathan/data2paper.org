...
function wp_ajax_crp_get_portfolio(){
    ...
    if(count($res)){
        ...
        foreach($res as $project){
            ...
            $picId = $picJson->id;
            $picInfo = CRPHelper::getAttachementMeta($picId, "medium");
            $pic = array(
              "id" => $picId,
              "src" => $picInfo["src"],
            );
...
function wp_ajax_crp_get_options(){
    global $wpdb;
    $response = new stdClass();
    ...
    if(count($res)){
        ...
        //die($portfolio->options);
        if($portfolio->options && !empty($portfolio->options)){
            $response->options = $portfolio->options;
        }else{
            $response->options = CRPHelper::getPortfolioDefaultOptions();
        }

        $response->status = 'success';
    }else{
        $response->status = 'error';
        $response->errormsg = 'Portfolio was not found!';
    }

    crp_ajax_return($response);
}

function wp_ajax_crp_save_options() {
    global $wpdb;
    $response = new stdClass();

    if(!isset($_POST['options']) || !isset($_POST['pid'])){
        $response->status = 'error';
