...
function crp_init_action() {
    ...
    if (isset($_GET['theme']) && ($_GET['theme'] == 'dark' || $_GET['theme'] == 'light')) {
        update_option(CRP_THEME_OPTION, $_GET['theme']);
    }
...
function crp_enqueue_admin_scripts(){
    ...
    wp_register_script('jquery-accordion', CRP_JS_URL."/jquery/jquery.accordion.js", array('jquery'), "", true);
...
function crp_enqueue_admin_csss(){
    //Enqueue CSS files
    global $crp_theme;
    $crp_theme = get_option(CRP_THEME_OPTION, CRP_DEFAULT_THEME);

    if ($crp_theme == 'dark') {
        wp_register_style('crp-main-admin-style', CRP_CSS_URL.'/crp-main-admin_dark.css');
    } else {
